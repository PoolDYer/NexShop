# Script automático para sincronizar carpetas con 50 productos reales
# Sin confirmación - ejecuta directamente

$basePath = "E:\Proyectos Visual\NexShop\NexShop.Web\wwwroot\uploads\productos"

Write-Host "Sincronizando carpetas con 50 productos reales..." -ForegroundColor Green
...
$productosReales = @{
    ...
    21 = "Juego de Sábanas 100% Algodón"
...
# Eliminar las que no correspondan a productos (IDs > 50)
$eliminadas = 0
...
Write-Host "Eliminadas: $eliminadas carpetas innecesarias" -ForegroundColor Red

# Crear/renombrar las 50 carpetas correctas
$creadas = 0
$renombradas = 0

foreach ($id in 1..50) {
    $nombre = $productosReales[$id]
    $carpetaEsperada = "$id`_$nombre"
    $rutaEsperada = Join-Path $basePath $carpetaEsperada
    
    if (-not (Test-Path $rutaEsperada)) {
        # Buscar si existe con otro nombre
        $carpetaActual = $carpetas | Where-Object { $_ -match "^$id`_" } | Select-Object -First 1
        
        if ($carpetaActual -and $carpetaActual -ne $carpetaEsperada) {
            # Renombrar
            try {
                $rutaActual = Join-Path $basePath $carpetaActual
                Rename-Item $rutaActual $carpetaEsperada
                $renombradas++
            } catch {}
        } else {
            # Crear
            try {
                New-Item -Path $rutaEsperada -ItemType Directory | Out-Null
                $creadas++
            } catch {}
        }
    }
...
Write-Host "Creadas: $creadas carpetas nuevas" -ForegroundColor Green
Write-Host "Renombradas: $renombradas carpetas" -ForegroundColor Yellow

# Verificar
$final = (Get-ChildItem -Path $basePath -Directory).Count
Write-Host ""
Write-Host "Carpetas finales: $final" -ForegroundColor Cyan
Write-Host "Esperadas: 50" -ForegroundColor Cyan

# ================================================
# FORZAR REGENERACIÓN DE IMÁGENES ESPECÍFICAS
# ================================================

Write-Host ""
Write-Host "?? FORZANDO REGENERACIÓN DE IMÁGENES ESPECÍFICAS" -ForegroundColor Cyan
Write-Host ""

$projectPath = "E:\Proyectos Visual\NexShop\NexShop.Web"
Set-Location $projectPath

# Opción 1: Eliminar archivo de base de datos para forzar recreación
Write-Host "? OPCIONES DE REGENERACIÓN:" -ForegroundColor Yellow
Write-Host ""
Write-Host "OPCIÓN 1 (RECOMENDADA): Eliminar BD y regenerar todo" -ForegroundColor Green
Write-Host "  - Elimina NexShopDB.mdf/.ldf" -ForegroundColor Gray
Write-Host "  - Regenera 50 productos con imágenes específicas" -ForegroundColor Gray
Write-Host "  - Tiempo: 2 minutos" -ForegroundColor Gray
Write-Host ""
Write-Host "OPCIÓN 2: Solo actualizar URLs de multimedia existente" -ForegroundColor Yellow
Write-Host "  - Mantiene productos y datos" -ForegroundColor Gray
Write-Host "  - Solo cambia URLs de imágenes" -ForegroundColor Gray
Write-Host "  - Tiempo: 30 segundos" -ForegroundColor Gray
Write-Host ""

$opcion = Read-Host "Elige opción (1 o 2)"

if ($opcion -eq "1") {
    Write-Host ""
    Write-Host "???  Eliminando base de datos..." -ForegroundColor Red
    
    # Buscar y eliminar archivos de BD
    $bdFiles = Get-ChildItem -Path $projectPath -Filter "*.mdf" -Recurse -ErrorAction SilentlyContinue
    $bdFiles += Get-ChildItem -Path $projectPath -Filter "*.ldf" -Recurse -ErrorAction SilentlyContinue
    ...
    foreach ($file in $bdFiles) {
        try {
            ...
            Write-Host "   ??  No se pudo eliminar: $($file.Name)" -ForegroundColor Yellow
        }
    }
    
    Write-Host ""
    Write-Host "?? Iniciando aplicación (regenerará TODO con imágenes específicas)..." -ForegroundColor Cyan
    Write-Host ""
    
} elseif ($opcion -eq "2") {
    Write-Host ""
    Write-Host "??  IMPLEMENTANDO OPCIÓN 2..." -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Esta opción requiere acceso directo a SQL Server." -ForegroundColor Gray
    Write-Host "Por favor, ejecuta manualmente en SQL Server Management Studio:" -ForegroundColor Gray
    Write-Host ""
    Write-Host @"
...
"@ -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "Presiona Enter para iniciar la aplicación cuando termines..." -ForegroundColor Yellow
    Read-Host
}
...
Write-Host ""
Write-Host "?? Abre tu navegador en: http://localhost:5217/Productos" -ForegroundColor Magenta
Write-Host "?? Verás imágenes específicas para cada producto" -ForegroundColor Green
Write-Host ""
